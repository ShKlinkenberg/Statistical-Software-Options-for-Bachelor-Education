...
In a short survey among the scientific staff of the department of communication science, we asked about the use of statistical software in their research and teaching. The results are shown in @tbl-software-use. In total we received 64 responses, which is a response rate of about 50%. The results show that the majority of the staff uses R (63%) and SPSS (67%) in their research. Python is used by 22%. Most staff indicate to be familiar with both R and SPSS, while the results also show that SPSS is sometimes used only for teaching and not for research.
...
software.use$Percentage <- round(software.use$Frequency / sum(software.use$Frequency) * 100, 1)
