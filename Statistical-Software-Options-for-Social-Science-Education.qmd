...
subtitle: "Recommendations and considerations for the Department of Communication Science at the University of Amsterdam"
...
In the following chapters, we will give an overview of the different software options available for undergraduate education, and provide considerations for their use. We will also provide recommendations for the implementation of statistical software in the curriculum.
...
Though both are based on R and can produce syntax for the analysis that are performed, both use their own packages. For example a t test in R would be run with the function `t.test()`, while in JASP and Jamovi the functions are `jaspTTests::TTestIndependentSamples()` and `jmv::ttestIS()` respectively. This means that the syntax produced by JASP and Jamovi is not directly usable in R. This is a disadvantage for students who want to transition to R in a masters program, as they will have to learn a different set of functions.
...
Use Cases:  
Best suited for bachelor or master courses focusing on advanced statistical analysis, reproducible research, and data science.
...
Use Cases:  
Suitable for advanced bachelor and master courses in data science, computational social science, and research with large, complex data sets.
